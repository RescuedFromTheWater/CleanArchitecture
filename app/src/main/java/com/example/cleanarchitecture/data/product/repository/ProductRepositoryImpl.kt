package com.example.cleanarchitecture.data.product.repository
...
import com.example.cleanarchitecture.domain.product.entity.ProductUserEntity
...
@Inject constructor(private val productApi: ProductApi) :
    ProductRepository {
    override suspend fun getAllMyProducts(
        ...
        return flow {
            ...
            if (response.isSuccessful) {
                ...
                body.data?.forEach { productResponse ->
                    user = ProductUserEntity(
                        productResponse.user.id,
                ...
                val err = Gson().fromJson<WrappedListResponse<ProductResponse>>(
    ...
    override suspend fun createProduct(productCreateRequest: ProductCreateRequest
    ): Flow<BaseResult<ProductEntity, WrappedResponse<ProductResponse>>> {
        return flow {
