...
import com.example.cleanarchitecture.databinding.FragmentMainDetailBinding
...
class DetailMainFragment : Fragment(R.layout.fragment_main_detail) {
    ...
    private fun handleState(state: DetailMainFragmentState){
        when(state){
            ...
            is DetailMainFragmentState.SuccessDelete ->
                findNavController().navigate(R.id.action_update_to_home)
            is DetailMainFragmentState.Init -> Unit
            is DetailMainFragmentState.ShowToast ->
                requireActivity().showToast(state.message)
            is DetailMainFragmentState.IsLoading ->
